...
import { Upload, X, Trash2, Lock } from 'lucide-react';
...
export function EditAlbumModal({ isOpen, album, onClose, onUpdateAlbum, language }: EditAlbumModalProps) {
  const t = getTranslation(language);
  const [title, setTitle] = useState(album.title);
  const [publisherName, setPublisherName] = useState(album.publisherName);
  const [publishDate, setPublishDate] = useState(album.publishDate);
  const [media, setMedia] = useState<MediaItem[]>(album.media);
  const [customCover, setCustomCover] = useState<string>(album.coverImage);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !publisherName || media.length === 0) {
      alert(t.fillAllFields);
      return;
    }

    // Validate password if album is locked
    if (isLocked && (changePassword || !album.isLocked)) {
    ...
    let passwordHash = album.passwordHash;
    if (isLocked && (changePassword || !album.isLocked)) {
    ...
    const updatedAlbum: Album = {
      ...
      publishDate,
      media,
      coverImage: customCover || media[0]?.url || '',
      isLocked,
      passwordHash,
    };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-white max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-pink-900">{t.editAlbum}</DialogTitle>
          <DialogDescription>
            {t.editAlbumDesc}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4 border border-pink-200 rounded-lg p-4 bg-pink-50/30">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="locked-edit" className="text-pink-900 flex items-center gap-2 cursor-pointer">
            ...
            {isLocked && (
              <div className="space-y-3 pl-6 border-l-2 border-pink-300">
                ...
                {album.isLocked && !changePassword ? (
                  ...
                  <>
                    <div>
                      <Label htmlFor="password-edit" className="text-pink-900">
                        {album.isLocked ? t.newPassword : t.password}
                      </Label>
                      <Input
                        id="password-edit"
                        type="password"
                        ...
                        className="border-pink-200 focus:border-pink-400"
                        required={isLocked && (changePassword || !album.isLocked)}
                      />
                    </div>
                    <div>
                      <Label htmlFor="confirmPassword-edit" className="text-pink-900">{t.confirmPassword}</Label>
                      <Input
          ...
          <div>
            ...
            {customCover ? (
              <div className="relative w-full aspect-[4/3] bg-pink-100 rounded-lg overflow-hidden mb-2">
              ...
              <div className="border-2 border-dashed border-pink-300 rounded-lg p-8 text-center bg-pink-50/50 hover:bg-pink-100/50 transition-colors mb-2">
                <input
                  type="file"
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-pink-300 rounded-lg p-8 text-center bg-pink-50/50 hover:bg-pink-100/50 transition-colors">
              ...
              <label htmlFor="media-upload-edit" className="cursor-pointer">
                ...
                <p className="text-pink-900 mb-1">{t.addMorePhotos}</p>
                ...
                {media.map((item, index) => (
                  <div key={item.id} className="relative group">
                    <div className="aspect-square bg-pink-100 rounded-lg overflow-hidden">
                      ...
                      {index === 0 && (
                        <div className="absolute top-1 left-1 bg-pink-500 text-white px-2 py-1 rounded text-xs">
                          Cover
                        </div>
                      )}
                    </div>
                    ...
                    <p className="text-pink-800 truncate mt-1 text-xs">{item.filename}</p>
                  </div>
                ))}
              </div>
              <p className="text-pink-600 mt-2">* First item will be used as the album cover</p>
            </div>
          )}
